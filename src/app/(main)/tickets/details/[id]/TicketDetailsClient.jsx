...
const Countdown = ({ target }) => {
        ...
        return (
            <div className="flex items-center gap-2">
                {["Days", "Hrs", "Min", "Sec"].map((unit) => (
                    ...
                    >
                        <span className="text-lg font-black leading-none tabular-nums text-emerald-600 dark:text-emerald-400">00</span>
                        <span className="text-[9px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 mt-1">{unit}</span>
                    </div>
                ))}
            </div>
        );
    ...
    if (diff === 0) {
        return (
            <Chip color="danger" className="font-semibold animate-pulse" variant="flat">
                <FaTimesCircle className="mr-1 inline" /> Departed
            </Chip>
        );
    }
            ...
            {timeUnits.map((unit, index) => (
                <div key={unit.label} className="flex items-center gap-2">
                    <div className="flex flex-col items-center justify-center rounded-lg bg-white/95 backdrop-blur-md px-3 py-2 shadow-lg min-w-[50px] border border-emerald-200/60 dark:bg-zinc-800/95 dark:border-emerald-500/30">
                        ...
                        <span className="text-[9px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 mt-1">{unit.label}</span>
                    </div>
                    {index < timeUnits.length - 1 && (
                        <span className="text-lg font-black text-white/80 dark:text-white/60 -mx-0.5 animate-pulse">:</span>
                    )}
                </div>
            ))}
        </div>
...
const TrustBadge = ({ icon: Icon, title, desc }) => (
    <div className="flex items-center gap-3 p-2 rounded-lg hover:bg-emerald-50/50 dark:hover:bg-emerald-500/5 transition-colors">
        ...
        <div className="min-w-0">
            ...
            <p className="text-[11px] text-zinc-500 dark:text-zinc-500">{desc}</p>
        </div>
    </div>
);
...
const TicketDetailsClient = ({ ticket, user }) => {
    ...
    const roleMsg = useMemo(() => {
        const role = (user?.role || "").toLowerCase();
        if (role === "admin") return "Admins cannot book tickets";
        if (role === "vendor") return "Vendors cannot book tickets";
        return null;
    }, [user]);
    ...
    const transportColor = transportColors[ticket?.transportType] || "from-emerald-500 to-green-600";
            ...
            <div className="container mx-auto px-4 relative z-10">
                ...
                >
                    <div className="relative h-[260px] w-full md:h-[380px]">
                        {ticket?.image ? (
                            <Image src={ticket.image} alt={ticket.title || "Ticket"} fill priority className="object-cover" />
                        ) : (
                        ...
                        >
                            <Icon className="text-emerald-600 dark:text-emerald-400" size={16} />
                            <span className="text-sm font-bold text-zinc-800 dark:text-zinc-100">{ticket?.transportType || "Bus"}</span>
                        </motion.div>
                        ...
                        >
                            <div className={`rounded-xl bg-gradient-to-r ${transportColor} px-4 py-2 shadow-lg`}>
                                <p className="flex items-center gap-1 text-white font-black text-lg">
                                    <TbCurrencyTaka size={20} />
                                    {ticket?.price?.toLocaleString()}
                                </p>
                                <p className="text-[10px] font-bold text-white/80 text-right -mt-0.5">Per Unit</p>
                            </div>
                        </motion.div>
                        ...
                        >
                            ...
                            <div className="flex flex-wrap items-center gap-3">
                                <div className="flex items-center gap-2 bg-black/50 backdrop-blur-md px-4 py-2 rounded-full border border-white/15">
                                    ...
                                    <FaArrowRight className="text-emerald-300" size={11} />
                                    <span className="flex items-center gap-1.5 text-sm font-bold text-white">
                    ...
                    <div className="grid gap-5 p-5 md:grid-cols-3 md:p-7">
                        <div className="space-y-5 md:col-span-2">
                            ...
                            >
                                <h2 className="mb-4 text-base font-black text-zinc-900 dark:text-white flex items-center gap-2">
                                    <FaMapMarkerAlt className="text-emerald-500" size={15} />
                                    Route Information
                                </h2>
                                <div className="flex items-center gap-4">
                                    <div className="flex-1 text-center">
                                        <div className="mx-auto mb-2 flex h-12 w-12 items-center justify-center rounded-full bg-emerald-100 ring-2 ring-emerald-200 dark:bg-emerald-500/15 dark:ring-emerald-500/30">
                                            <FaMapMarkerAlt className="text-emerald-600 dark:text-emerald-400" size={18} />
                                        </div>
                                        <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-bold uppercase tracking-wider">From</p>
                                        <p className="text-lg font-black text-zinc-800 dark:text-white mt-0.5">{ticket?.from || "N/A"}</p>
                                    </div>
                                    <div className="flex flex-col items-center">
                                        <div className="h-px w-12 bg-gradient-to-r from-emerald-300 to-green-300 dark:from-emerald-600 dark:to-green-600" />
                                        <FaArrowRight className="text-emerald-500 dark:text-emerald-400 my-2" size={18} />
                                    ...
                                    <div className="flex-1 text-center">
                                        <div className="mx-auto mb-2 flex h-12 w-12 items-center justify-center rounded-full bg-green-100 ring-2 ring-green-200 dark:bg-green-500/15 dark:ring-green-500/30">
                                            <FaMapMarkerAlt className="text-green-600 dark:text-green-400" size={18} />
                                        </div>
                                        <p className="text-[10px] text-green-600 dark:text-green-400 font-bold uppercase tracking-wider">To</p>
                                        <p className="text-lg font-black text-zinc-800 dark:text-white mt-0.5">{ticket?.to || "N/A"}</p>
                                    </div>
                                </div>
                            </motion.div>
                            ...
                            >
                                <h2 className="mb-4 text-base font-black text-zinc-900 dark:text-white flex items-center gap-2">
                                    <FaClock className="text-emerald-500" size={15} />
                                    Schedule
                                </h2>
                                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                    <div className="rounded-lg border border-zinc-100 bg-zinc-50 p-4 dark:border-zinc-700/50 dark:bg-zinc-800/60">
                                        <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 mb-1.5">Date</p>
                                        <div className="flex items-center gap-2.5">
                                            <FaCalendarAlt className="text-emerald-500 dark:text-emerald-400 shrink-0" size={15} />
                                            <p className="text-sm font-bold text-zinc-800 dark:text-zinc-100">{formattedDate || "N/A"}</p>
                                        </div>
                                    </div>
                                    <div className="rounded-lg border border-zinc-100 bg-zinc-50 p-4 dark:border-zinc-700/50 dark:bg-zinc-800/60">
                                        <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400 mb-1.5">Time</p>
                                        <div className="flex items-center gap-2.5">
                                            <FaClock className="text-emerald-500 dark:text-emerald-400 shrink-0" size={15} />
                                            <p className="text-sm font-bold text-zinc-800 dark:text-zinc-100">{formattedTime || "N/A"}</p>
                                        </div>
                                    </div>
                                </div>
                            </motion.div>
                            ...
                            <motion.div
                                ...
                                className="rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-500/60 dark:bg-zinc-800/50"
                            >
                                <h2 className="mb-4 text-base font-black text-zinc-900 dark:text-white">Vendor Information</h2>
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="font-bold text-base text-zinc-800 dark:text-white">{ticket?.vendorName || "Vendor"}</p>
                                        <p className="text-sm text-zinc-500 dark:text-zinc-400">{ticket?.vendorEmail || ""}</p>
                                    </div>
                                </div>
                            </motion.div>
                        </div>
                        ...
                        <div className="md:col-span-1">
                            <div className="sticky top-24 space-y-4">
                                ...
                                >
                                    <h2 className="text-lg font-black text-zinc-900 dark:text-white mb-4">Book This Ticket</h2>
                                    ...
                                    <div className="mb-4 rounded-xl bg-gradient-to-r from-emerald-50 to-green-50 p-4 border border-emerald-100 dark:from-emerald-500/10 dark:to-green-500/10 dark:border-emerald-500/25">
                                        <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">Price Per Unit</p>
                                    ...
                                    <div className="mb-4 flex items-center justify-between rounded-lg bg-zinc-50 px-4 py-3 border border-zinc-100 dark:bg-zinc-700/40 dark:border-zinc-600/50">
                                        <span className="text-sm font-semibold text-zinc-600 dark:text-zinc-300">Available Seats</span>
                                        <span className={`text-xl font-black ${available <= 5 ? "text-red-600 dark:text-red-400" : "text-emerald-600 dark:text-emerald-400"}`}>
                                            {available}
                                        </span>
                                    </div>
                                    ...
                                    <AnimatePresence>
                                        {soldOut && (
                                            ...
                                            >
                                                <FaTimesCircle className="animate-pulse" size={14} />
                                        ...
                                        {departed && !soldOut && (
                                            ...
                                            >
                                                <FaTimesCircle className="animate-pulse" size={14} />
                                                <span className="text-sm font-bold">Departure Passed</span>
                                            </motion.div>
                                        )}
                                        ...
                                        {isBooked && (
                                            ...
                                            >
                                                ...
                                                <span className="text-sm font-bold">Already Booked</span>
                                            </motion.div>
                                        )}

                                        {!canBook && !soldOut && !departed && !isBooked && (
                                            <motion.div
                                                initial={{ opacity: 0, y: -10 }}
                                                animate={{ opacity: 1, y: 0 }}
                                                exit={{ opacity: 0, y: -10 }}
                                                className="mb-4 flex items-center gap-2 rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/25"
                                            >
                                                <FaTimesCircle size={14} />
                                                <span className="text-sm font-bold">{roleMsg}</span>
                                            </motion.div>
                                        )}
                                    </AnimatePresence>
                                    ...
                                    <p className="mt-3 text-[11px] text-zinc-500 dark:text-zinc-500 text-center leading-relaxed">
                                        Booking will be sent to vendor as pending. Pay after acceptance.
                                    </p>
                                </motion.div>
                                ...
                                >
                                    <div className="space-y-1">
                                        <TrustBadge icon={FaShieldAlt} title="Secure Payment" desc="Your data is protected" />
                                        <TrustBadge icon={FaUndo} title="Easy Cancellation" desc="Cancel before acceptance" />
                                        <TrustBadge icon={FaHeadset} title="24/7 Support" desc="We're here to help" />
                                    </div>
                                </motion.div>
                            </div>
                        </div>
                        ...
                        >
                            <div className="relative bg-gradient-to-br from-emerald-500 to-green-600 p-5 overflow-hidden">
                                ...
                                <div className="relative flex items-center justify-between">
                                    <div>
                                        <h2 className="text-xl font-black text-white">Book Ticket</h2>
                                        <p className="text-sm text-emerald-100 mt-0.5">{ticket?.title}</p>
                                    </div>
                                    <motion.button
                                ...
                                <div className="relative flex items-center gap-2 mt-3 text-white bg-white/15 rounded-lg p-3 backdrop-blur-sm">
                                    <FaMapMarkerAlt size={13} className="text-emerald-200" />
                                    <span className="text-sm font-bold">{ticket?.from}</span>
                                    <FaArrowRight size={11} />
                                    <FaMapMarkerAlt size={13} className="text-green-200" />
                                    <span className="text-sm font-bold">{ticket?.to}</span>
                                </div>
                            </div>
                            ...
                            <div className="p-5 space-y-4">
                                <div className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-800/60">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Unit Price</p>
                                        ...
                                        <div className="text-right">
                                            <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Available</p>
                                            <p className="text-xl font-black text-zinc-900 dark:text-white">{available}</p>
                                        </div>
                                    </div>
                                </div>

                                <div>
                                    <label className="block text-sm font-bold text-zinc-700 dark:text-zinc-200 mb-2">Number of Tickets</label>
                                    <div className="flex items-center gap-2">
                                        <motion.button
                                            whileTap={{ scale: 0.9 }}
                                            type="button"
                                        ...
                                        <div className="flex-1">
                                            <input
                                                ...
                                                onChange={(e) => {
                                                    ...
                                                    if (v < 1) setQty(1);
                                                    else if (v > available) setQty(available);
                                                    else setQty(v);
                                                }}
                                ...
                                <div className="rounded-lg border-2 border-emerald-200 bg-gradient-to-r from-emerald-50 to-green-50 p-4 dark:border-emerald-500/30 dark:from-emerald-500/10 dark:to-green-500/10">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">Total Amount</p>
                                            <p className="text-3xl font-black text-emerald-700 dark:text-emerald-300 flex items-center">
                                                <TbCurrencyTaka size={28} />
