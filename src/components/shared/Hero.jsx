...
const HeroSlider = () => {
    return (
        <section className="w-full mt-16 rounded-b-3xl overflow-hidden shadow-2xl shadow-green-900/10 group">
            <Swiper
                modules={[
                    Autoplay,
                    Pagination,
                ...
                speed={800}
                className="w-full min-h-[65vh] md:min-h-[75vh] lg:min-h-[88vh]"
            >
                {heroSlides.map((slide, index) => (
                    <SwiperSlide key={slide.id} className="relative overflow-hidden">
                        {/* Next.js Image with subtle slow zoom */}
                        <Image
                            src={slide.bgImage}
                            alt={slide.title}
                            fill
                        ...
                        {/* Content */}
                        <div className="relative z-20 h-full min-h-[65vh] md:min-h-[75vh] lg:min-h-[88vh] max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-center py-16 md:py-0">
                            {/* Fixed width-fit Chip (no full width) */}
                            <Chip
                                variant="shadow"
                                color="success"
                                radius="full"
                                size="lg"
                                className="mb-5 w-fit !bg-white/10 backdrop-blur-xl border border-white/20 text-white font-medium hover:scale-[1.02] transition-all"
                            >
                                {slide.badge}
                            </Chip>

                            {/* Title WITH TEXT SHADOW CLASS APPLIED */}
                            <h1 className="text-[clamp(1.8rem,5vw,3.7rem)] font-bold text-white leading-[1.2] max-w-3xl mb-5 text-shadow">
                                {slide.title}
                            </h1>

                            {/* Subtitle WITH TEXT SHADOW CLASS APPLIED */}
                            <p className="text-base sm:text-lg text-zinc-200 max-w-2xl mb-8 leading-relaxed text-shadow">
                                {slide.subtitle}
                            </p>
                ...
                <div className="hero-pagination absolute bottom-6 left-0 right-0 z-30 flex justify-center" />
            </Swiper>
        </section>
    );
};
